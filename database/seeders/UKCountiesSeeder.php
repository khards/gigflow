...
/**
 * Contains the db seeder with the provinces of the United Kingdom.
 *
 * @copyright   Copyright (c) 2021 Keith Hards
 * @author      Keith Hards
 * @license
 * @since       2021-03-17
 */
...
class UKCountiesSeeder extends Seeder
{
    public function run()
    {
        $regions = [];
        $counties = str_getcsv(self::CSV);
        foreach ($counties as $row) {
            $regions[$row[1]] = $row[1];
        }

        foreach ($regions as $region) {
            $regions[$region]['id'] = \DB::table('provinces')->insert([
                [
                    'code' => null,
                    'name' => $region,
                    'country_id' => 'GB',
        ...
        foreach ($counties as $row) {
            \DB::table('provinces')->insert([
                [
                    ...
                    'name' => $row[0],
                    'country_id' => 'GB',
                    'parent_id' => array_search($row[0], $regions),
