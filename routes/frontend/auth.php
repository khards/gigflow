...
use App\Domains\Payment\Http\Controllers\OrderPaymentController;
use Tabuna\Breadcrumbs\Trail;

// **********************************
// Payment routes, no auth required.
// **********************************
// Payment via PayPal is via Javascript. Some data is sent to the JS on page load in the HTML.
// IPN's receive the actual payment information, so no form posts or API calls to the App are made.
Route::get('order/payment/{order}', [OrderPaymentController::class, 'show'])
    ->name('order.payment');

// *****************************************
// frontend.external.ical, no auth required.
// External calendar feed using users uuid
// *****************************************
Route::get('ical/{uuid}', function($uuid) {
    $ctrl = new \App\Booking\Controllers\Frontend\CalendarController();
    return $ctrl->getIcal($uuid);
})->name('external.ical');
